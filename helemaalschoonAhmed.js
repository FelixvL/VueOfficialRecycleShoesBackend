...
// Een get statement met een recordset terug
app.get('/getalldeliveriessdb',(request, response)=>{
  pool.query('select * from delivery', function (err, recordset) {
    if (err){ 
      console.log(err)
    }
    response.send(recordset);
...
// Een post-statement met een korte bevestiging
app.post('/deliverytoevoegen',(req, res)=>{
  console.log("test dat post lukt: "+req.body.voornaam);
  pool.query(`insert into delivery(id, product, datedelivery, administrator) VALUES (NULL, '${req.body.product}', '${req.body.datedelivery}','${req.body.administrator}')`, (e,r,f)=>{
    if(e){
      return console.log(e);
    }
    return console.log(r);
  })
  res.send("delivery toegevoegd");
});
